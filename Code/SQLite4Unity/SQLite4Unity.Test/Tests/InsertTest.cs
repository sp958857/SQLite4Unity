...
{    
    ...
    public class InsertTest
    {
		...
		{
			var testObjects = Enumerable.Range(1, 3).Select(i => new UniqueObj { Id = i }).ToList();
		    _db.TableChanged += (obj, e) =>
		    {
		        if (e.Table.TableName == typeof(UniqueObj).Name)
		        {
		            Console.WriteLine("changed:" + _db.IsInTransaction);
                }
            };
		    _db.InsertAll(testObjects);
